...
import { useMockRevenue } from "@/hooks/useMockData";
...
export function RevenueTracker() {
  ...
  const mockRevenue = useMockRevenue();

  const total = mockRevenue.total * 100; // cents
  const byProduct = mockRevenue.byProduct.map(p => ({
    product: p.product,
    amount: p.amount * 100
  }));
  const summaryData = mockRevenue.byPeriod.map(p => ({
    period: p.period,
    amount: p.amount * 100
  }));
  ...
  const maxAmount = Math.max(...summaryData.map(d => d.amount), 1);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          <div className="font-mono text-lg text-white">{formatCurrency(total)}</div>
        </div>
      </div>
