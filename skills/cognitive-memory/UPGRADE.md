...
| Version | Key Changes |
|---------|-------------|
| 1.0.0 | Initial release — multi-store memory, triggers, decay, reflection |
...
| 1.0.4 | Conversational flow, element menu, user permission triggers |
| 1.0.5 | Internal monologue format, honesty rule, dark humor, self-awareness tags |
...
1. **Full Reflection Archive** — Permanent storage of complete reflections
2. **IDENTITY.md** — Facts + Self-Image + Self-Awareness Log
3. **SOUL.md** — Expanded template with Values, Principles, Commitments
4. **Self-Awareness Tagging** — `[Self-Awareness]` mechanism
5. **Self-Image Consolidation** — Rewrites based on accumulated evidence

### Compatibility

- ✅ Backward compatible with existing memory stores
- ✅ Existing reflections preserved
- ⚠️ New files need to be created (IDENTITY.md, SOUL.md)
- ⚠️ decay-scores.json needs new fields
...
bash /path/to/cognitive-memory/scripts/upgrade_to_1.0.6.sh
```

Or if installed via ClawHub:
```bash
bash ~/.openclaw/skills/cognitive-memory/scripts/upgrade_to_1.0.6.sh
```
...
# Create reflections archive directory
